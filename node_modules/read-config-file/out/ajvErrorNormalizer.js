...
class SchemeErrorPrinter {
    ...
    getSchemaPartText(schemaPart) {
        let additionalPath = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    ...
    formatSchema(schema, prevSchemas) {
        ...
        const formatInnerSchema = function (innerSchema) {
            let isAddSelf = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;

            if (!isAddSelf) {
                return _this.formatSchema(innerSchema, prevSchemas);
            }
            if (prevSchemas.indexOf(innerSchema) >= 0) {
                return "(recursive)";
            }
            return _this.formatSchema(innerSchema, prevSchemas.concat(schema));
        };
...
function filterErrors(errors) {
    ...
    for (const error of errors) {
        ...
        newErrors = newErrors.filter(oldError => {
            if (!(oldError.dataPath.indexOf(dataPath) !== -1)) {
                return true;
            }
