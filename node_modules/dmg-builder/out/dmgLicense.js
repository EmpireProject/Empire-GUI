...
exports.addLicenseToDmg = exports.debug = undefined;
...
let addLicenseToDmg = exports.addLicenseToDmg = (() => {
    var _ref = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (packager, dmgPath) {
        ...
        const licenseButtonFiles = yield (0, (_licenseButtons || _load_licenseButtons()).getLicenseButtonsFile)(packager);
        ...
        for (const item of licenseFiles) {
            (0, (_builderUtil || _load_builderUtil()).log)("Adding " + item.langName + " license");
  ...
  $"0000 0000 0000"
            ...
            const fileData = yield (0, (_fsExtraP || _load_fsExtraP()).readFile)(item.file, "utf-8");
            const isRtf = item.file.endsWith(".rtf") || item.file.endsWith(".RTF");
            data += isRtf ? (0, (_dmgUtil || _load_dmgUtil()).serializeString)(Buffer.from(fileData).toString("hex")) : wrapInRtf((yield (0, (_fsExtraP || _load_fsExtraP()).readFile)(item.file, "utf-8")));
            data += "\n};";
            rtfs.push(data);
            defaultButtons.push((yield (0, (_licenseButtons || _load_licenseButtons()).getLicenseButtons)(licenseButtonFiles, item.langWithRegion, counter, item.langName)));
        ...
        const lPic = `data 'LPic' (5000) {\n${(0, (_dmgUtil || _load_dmgUtil()).serializeString)(buffer.toString("hex"))}\n};`;
        ...
        yield (0, (_builderUtil || _load_builderUtil()).exec)("hdiutil", ["unflatten", dmgPath]);
        yield (0, (_builderUtil || _load_builderUtil()).exec)("Rez", ["-a", tempFile, "-o", dmgPath]);
...
function _load_licenseButtons() {
    return _licenseButtons = require("./licenseButtons");
}
...
${(0, (_dmgUtil || _load_dmgUtil()).serializeString)("63663020" + Buffer.from(getRtfUnicodeEscapedString(text)).toString("hex").toUpperCase() + "7D")}`;
